...
"%VENV_PYTHON%" -c "import flask, langchain_chroma, langchain_huggingface, chromadb, fitz, sentence_transformers, bertopic, umap, hdbscan, sklearn" >nul 2>&1
...
echo    Installing thematisation libraries ^(BERTopic, UMAP, HDBSCAN^)...
"%VENV_PYTHON%" -m pip install bertopic umap-learn hdbscan scikit-learn --quiet --no-warn-script-location
if %ERRORLEVEL% NEQ 0 (
    echo    WARNING: BERTopic install failed - thematisation unavailable.
    echo    Run manually: pip install bertopic umap-learn hdbscan scikit-learn
)
